...
$.fn.scroll = function (options) {
  const defaults = $.extend({
    items: [], // 数据
    scrollable: '.picker-content', // 滚动的元素
  ...
  const items = defaults.items.map((item) => {
    return `<div class="picker-item${item.disabled ? ' picker-item-disabled' : ''}">${typeof item == 'object' ? item.label : item}</div>`;
  }).join('');
  const $this = $(this);

  $this.find('.picker-content').html(items);
