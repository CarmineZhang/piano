...
<div class="<%= className %>">
    ...
    <div class="picker">
        <div class="picker-hd">
            <a href="javascript:;" data-action="cancel" class="picker-action">取消</a>
            <a href="javascript:;" data-action="select" class="picker-action" id="picker-confirm">确定</a>
        </div>
        <div class="picker-bd"></div>
    </div>
</div>
`
var groupTpl = `
...
function picker() {
  ...
  let depth = options.depth || (isMulti ? items.length : depthOf(items[0])),
    groups = '';
  ...
  function show() {
    ...
    $picker.find('.mask').addClass('animate-fade-in');
    $picker.find('.picker').addClass('animate-slide-up');
  }

  var _hide = function (callback) {
    _hide = $.noop; // 防止二次调用导致报错

    $picker.find('.mask').addClass('animate-fade-out');
    $picker.find('.picker')
      .addClass('animate-slide-down')
  ...
  function scroll(items, level) {
    ...
    $picker.find('.picker-group').eq(level).scroll({
      items: items,
      temp: lineTemp[level],
  ...
  $picker.find('.picker-bd').html(groups);
  show();
  ...
  $picker
    ...
    .on('click', '.picker-action', function () {
      hide();
    })
    .on('click', '#picker-confirm', function () {
